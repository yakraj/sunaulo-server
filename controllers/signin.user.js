const signinhandler = (db,bcrypt) => (req,res) => {
	...
	db.select('mobile', 'password', 'username').from('usercrediantials').where('mobile', '=', req.body.mobile)
	.then(data => {
    ...
    .where("ui.username", data[0].username).then(respo => res.json(respo))
			.catch(err => res.status(400).json('Wrong Mobile or Password'))
		}else{res.status(400).json('Wrong Mobile or Password')}
	}).catch(err => res.status(400).json('Wrong Mobile or Password'))
...
const NumberValidateHandler = (db)=> (req, res)=> {
	db.select('*').from("usercrediantials").where ('mobile', req.body.mobile)
