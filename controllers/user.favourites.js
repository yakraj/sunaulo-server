const getFavouritesHandler = (db) => (req, res) => {
  db.select("hearts")
    ...
    .catch((err) => res.status(404).json("unable to get favourites"));
};
...
const updateFavouritesHandler = (db) => (req, res) => {
  db("userfavourites")
    ...
    .catch((err) => res.status(404).json("unable to update favourites"));
};
...
const getFavouriteadsHandler = (db) => (req, res) => {
  ...
  db.raw(
    'select "title", "thumbnail", "address", "price", "supercatogery", "adid" from archive where adid in (' +
      myArray.map((_) => "?").join(",") +
      ")",
    [...myArray]
  )
    ...
    .catch((err) => res.status(404).json("unable to get any related data"))
