const getFilterKeysHandler = (db) => (req, res) => {
  function FilterClean(obj) {
    for (var propName in obj) {
      if (
        ...
        delete obj[propName];
      } else if (propName == "thumbnail") {
        delete obj[propName];
      } else if (propName == "price") {
        delete obj[propName];
      } else if (propName == "location") {
        delete obj[propName];
      } else if (propName == "catogery") {
        delete obj[propName];
      } else if (propName == "subcatogery") {
        delete obj[propName];
      } else if (propName == "adid") {
        delete obj[propName];
  ...
  db.select("*")
    ...
    .then((response) => {
      response.map(
        (nn, i) =>
          (newData = Object.keys(response[i]) && FilterClean(response[i]))
      );
      response.map((val, i) => {
